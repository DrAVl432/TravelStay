...
const SupportChatService = {
  async getMessages(chatId: string): Promise<{ isActive: boolean; messages: ChatMessage[] }> {
    const response = await fetch(`${API_URL}/api/support-requests/${chatId}/messages`);
    if (!response.ok) throw new Error(`Ошибка при получении сообщений: ${response.statusText}`);
  ...
  async sendMessage(chatId: string, author: string, text: string): Promise<void> {
    ...
    if (!response.ok) {
      ...
      throw new Error(textResp || `Ошибка при отправке сообщения: ${response.statusText}`);
    }
  },
  ...
  async getClientSupportRequests(userId: string): Promise<any[]> {
    ...
    if (!response.ok) throw new Error(`Ошибка при получении запросов клиента: ${response.statusText}`);
    return await response.json();
  },

  async getManagerSupportRequests(): Promise<any[]> {
    const response = await fetch(`${API_URL}/api/support-requests/manager-requests`);
    if (!response.ok) throw new Error(`Ошибка при получении запросов менеджера: ${response.statusText}`);
    return await response.json();
  },
  ...
  async createSupportRequest(userId: string, text: string): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Ошибка при создании обращения: ${response.statusText}`);
  },
  ...
  async markMessagesAsRead(userId: string, supportRequestId: string, createdBefore: Date): Promise<void> {
    ...
    if (!response.ok) throw new Error(`Ошибка при пометке сообщений как прочитанных: ${response.statusText}`);
  },

  async getUnreadCount(chatId: string, userId: string): Promise<number> {
    const response = await fetch(`${API_URL}/api/support-requests/${chatId}/unread-count?userId=${encodeURIComponent(userId)}`);
    if (!response.ok) throw new Error(`Ошибка при получении количества непрочитанных сообщений: ${response.statusText}`);
    const data = await response.json();
    return data.unreadCount;
  },

  async closeRequest(chatId: string): Promise<void> {
    const response = await fetch(`${API_URL}/api/support-requests/${chatId}/close`, { method: 'DELETE' });
    if (!response.ok) throw new Error(`Ошибка при закрытии обращения: ${response.statusText}`);
  },

  async getSupportRequestDetails(chatId: string): Promise<{ isActive: boolean; firstMessage: string }> {
    const response = await fetch(`${API_URL}/api/support-requests/${chatId}`);
    if (!response.ok) throw new Error(`Ошибка при получении деталей обращения: ${response.statusText}`);
    return await response.json();
  },
};
